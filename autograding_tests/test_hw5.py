...
def test_minimization():
    with open(f"{hw_path}/minimized_fsas.pkl", 'rb') as f:
        minimized_fsas: list[(FSA, FSA)] = pickle.load(f)
...
def test_minimization_example_Real():
    from rayuela.base.semiring import Real

    fsa = FSA(Real)
    fsa.set_I(State('a'), Real(0.5))

    fsa.add_arc(State('a'), '1', State('b'), Real(0.125))
    fsa.add_arc(State('a'), '2', State('c'), Real(0.25))
    fsa.add_arc(State('b'), '1', State('a'), Real(0.25))
    fsa.add_arc(State('b'), '2', State('d'), Real(0.25))
    fsa.add_arc(State('c'), '1', State('e'), Real(0.125))
    fsa.add_arc(State('d'), '1', State('e'), Real(0.125))
    fsa.add_arc(State('e'), '1', State('e'), Real(0.25))
