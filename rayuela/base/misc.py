...
from rayuela.base.semiring import Rational
    ...
    for q in fsa.Q:
        ...
        if state_class_name == "MinimizeState":
            ...
            for l, p, w in fsa.arcs(q):
                target = (
                    f"{state_class_name}({[f'State({i})' for i in p.idx]})".replace(
                        "'", ""
                    )
                )
                print(
                    f"{fsa_name}.add_arc({origin}, '{l}', {target}, {semiring_class_name}({weight_wrapper(w)}))"
                )
            for i, w in fsa.I:
                if q == i:
                    state = f"{[f'State({i})' for i in q.idx]}".replace("'", "")
                    print(
                        f"{fsa_name}.set_I({state_class_name}({state}), {semiring_class_name}({weight_wrapper(w)}))"
                    )
            for i, w in fsa.F:
                if q == i:
                    state = f"{[f'State({i})' for i in q.idx]}".replace("'", "")
                    print(
                        f"{fsa_name}.add_F({state_class_name}({state}), {semiring_class_name}({weight_wrapper(w)}))"
                    )
